...
encutToEnmaxRatios = [0.8, 1.0, 1.2, 1.3, 1.4, 1.5]
kSpacings = [0.50, 0.45, 0.40, 0.35, 0.30, 0.30]
eDiffs = [5e-3, 2e-3, 1e-3, 5e-4, 2e-4, 1e-4]
eDiffgs = [10*eDiff for eDiff in eDiffs]
isifs = [3, 3, 3, 3, 3, 0]
ibrions = [2, 1, 1, 1, 1, -1] 
nsws = [200, 100, 80, 60, 50, 0]
precs = ['low', 'Normal', 'Normal', 'Normal', 'Normal', 'High']
incarDict={}
...
for file in listOfStructureFiles :
    ...
    for incarIndex,encutToEnmaxRatio in enumerate(encutToEnmaxRatios) :
      incarDict["SYSTEM"] = fileName+', step '+str(incarIndex)
